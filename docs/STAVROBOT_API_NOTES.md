...
6. Because Shelley renders markdown and screenshots well, the bridge should be treated as a structured/rich-output boundary too, not only as a plain-text adapter.
7. `smoke-test-stavrobot-client.sh` exercises health, chat, listing, history, and events together against a live stack.

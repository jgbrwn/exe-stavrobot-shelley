...
- `patches/shelley/series/` now captures the first repo-owned cleaned managed patch series for the eventual Shelley-side implementation split, and the split `0001` → `0004` prototype patches have now been replay/apply validated in order against a fresh upstream Shelley checkout

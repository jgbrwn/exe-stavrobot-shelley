...
- `install-stavrobot.sh` now exposes explicit Shelley-mode entrypoints for the current managed flow via `--print-shelley-mode-status` and `--refresh-shelley-mode`, and now rejects ambiguous mixes with normal installer mutation flags
